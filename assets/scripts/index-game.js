...
if (window.mainColor == null) {
  window.mainColor = parseInt(
    localStorage.getItem("iconMainColor") || "04FF00",
    16,
  );
}
if (window.secondaryColor == null) {
  window.secondaryColor = parseInt(
    localStorage.getItem("iconSecondaryColor") || "00FBFF",
    16,
  );
}
...
window.currentBall =
  localStorage.getItem("iconCurrentBall") || "player_ball_01";
  ...
  preload() {
    ...
    this.load.on("filecomplete", (key, type) => {
      ...
      if (consoleLines.length > 0)
        consoleLines[consoleLines.length - 1] = `  ${key}  [${sizeMB} MB]`;
      pushLine("");
    });
    ...
    this.load.atlas(
      "GJ_WebSheet",
      "assets/sheets/GJ_WebSheet.png",
      "assets/sheets/GJ_WebSheet.json",
    );
    ...
    this.load.atlas(
      "GJ_GameSheet",
      "assets/sheets/GJ_GameSheet.png",
      "assets/sheets/GJ_GameSheet.json",
    );
    this.load.atlas(
      "GJ_GameSheet02",
      "assets/sheets/GJ_GameSheet02.png",
      "assets/sheets/GJ_GameSheet02.json",
    );
    this.load.atlas(
      "GJ_GameSheet03",
      "assets/sheets/GJ_GameSheet03.png",
      "assets/sheets/GJ_GameSheet03.json",
    );
    this.load.atlas(
      "GJ_GameSheet04",
      "assets/sheets/GJ_GameSheet04.png",
      "assets/sheets/GJ_GameSheet04.json",
    );
    this.load.atlas(
      "GJ_GameSheetEditor",
      "assets/sheets/GJ_GameSheetEditor.png",
      "assets/sheets/GJ_GameSheetEditor.json",
    );
    this.load.atlas(
      "GJ_GameSheetGlow",
      "assets/sheets/GJ_GameSheetGlow.png",
      "assets/sheets/GJ_GameSheetGlow.json",
    );
    this.load.atlas(
      "GJ_GameSheetIcons",
      "assets/sheets/GJ_GameSheetIcons.png",
      "assets/sheets/GJ_GameSheetIcons.json",
    );
    this.load.atlas(
      "GJ_LaunchSheet",
    ...
    this.load.atlas(
      "player_ball_00",
      "assets/sheets/player_ball_00.png",
      "assets/sheets/player_ball_00.json",
    );
    this.load.atlas(
      "player_dart_00",
      "assets/sheets/player_dart_00.png",
      "assets/sheets/player_dart_00.json",
    );
    ...
    for (let i = 1; i < 23; i++) {
      ...
      this.load.image(
        "groundSquare_" + paddedIndex + "_001.png",
        "assets/game-ground/groundSquare_" + i + "_001.png",
      );
    }
    ...
    for (let i = 1; i < 60; i++) {
      ...
      this.load.image(
        "game_bg_" + index,
        "assets/game-bg/game_bg_" + i + "_001-hd.png",
      );
    }
    ...
    for (const lvlarray of window.allLevels) {
      this.load.text(
        lvlarray[2],
        "assets/levels/" + lvlarray[2].split("_")[1] + ".txt",
      );
      this.load.audio(
        lvlarray[0],
        "assets/music/" +
          (lvlarray[4] ? lvlarray[4] : lvlarray[1].replaceAll(" ", "")) +
          ".mp3",
      );
    }
...
function loadFont(scene, fontName, fontData) {
  ...
  for (const line of fontData.split("\n")) {
    ...
    for (let i = 1; i < lineParts.length; i++) {
      ...
      if (equalIndex >= 0) {
        properties[lineParts[i].slice(0, equalIndex)] = lineParts[i]
          .slice(equalIndex + 1)
          .replace(/^"|"$/g, "");
      }
    }
  ...
  for (const kerningPair of kerningPairs) {
    if (fontConfig.chars[kerningPair.second]) {
      fontConfig.chars[kerningPair.second].kerning[kerningPair.first] =
        kerningPair.amount;
    }
  }
...
class PracticeMode {
  ...
  saveCheckpoint(playerState, playerWorldX, cameraX, scene) {
    ...
    const checkpointSprite = scene.add
      .image(
        playerWorldX,
        b(playerState.y),
        "GJ_GameSheet02",
...
function parseLevel(levelString) {
  let decompressedString = (function (compressedString) {
    let getBase64 = (function (compressedString) {
      let lessCluttered = compressedString
        .replace(/-/g, "+")
        .replace(/_/g, "/");
...
class Easing {
  ...
  static _bounceInOut(x) {
    return x < 0.5
      ? (1 - this._bounceTime(1 - x * 2)) * 0.5
      : this._bounceTime(x * 2 - 1) * 0.5 + 0.5;
  }
  static _bounceIn(x) {
  ...
  static _expInOut(x) {
    return x < 0.5
      ? 0.5 * Math.pow(2, 10 * (x * 2 - 1))
      : 0.5 * (-Math.pow(2, -10 * (x * 2 - 1)) + 2);
  }
  static _expIn(x) {
  ...
  static _backInOut(x) {
    const ov = 1.70158 * 1.525;
    const t = x * 2;
    return t < 1
      ? (t * t * ((ov + 1) * t - ov)) / 2
      : ((t - 2) * (t - 2) * ((ov + 1) * (t - 2) + ov)) / 2 + 1;
  }
  static _backIn(x) {
    const ov = 1.70158;
...
class us {
  ...
  loadLevel(levelData) {
    let { objects: levelObjects, settings: settingslist } =
  ...
  _buildGround() {
    ...
    const groundFrame = scene.textures.getFrame(
      "groundSquare_" + window._groundId + "_001.png",
    );
    ...
    for (let i = 0; i < tileCount; i++) {
      ...
      let ceilingTile = scene.add.image(
        0,
        groundY,
        "groundSquare_" + window._groundId + "_001.png",
      );
    ...
    const floorLineFrame = scene.textures.getFrame(
      "GJ_WebSheet",
    ...
    this._groundShadowL = scene.add
      .image(-1, groundY, "GJ_WebSheet", "groundSquareShadow_001.png")
      .setOrigin(0, 0)
      .setScrollFactor(0)
      .setDepth(22)
  ...
  resizeScreen() {
    ...
    while (this._groundTiles.length < requiredTileCount) {
      ...
      console.log(window._groundId);
      let newGroundTile = scene.add.image(
        0,
        groundY,
        "groundSquare_" + window._groundId + "_001.png",
      );
      newGroundTile
        .setOrigin(0, 0)
        .setTint(
          ((newTile = this._groundTiles[0]) == null
            ? undefined
            : newTile.tintTopLeft) || 17578,
        )
        .setDepth(20);
      newGroundTile._worldX = newTileX;
      this._groundTiles.push(newGroundTile);
      let newCeilingTile = scene.add.image(
        0,
        groundY,
        "groundSquare_" + window._groundId + "_001.png",
      );
      newCeilingTile
        ...
        .setFlipY(true)
        .setTint(
          ((newCeilingTile = this._groundTiles[0]) == null
            ? undefined
            : newCeilingTile.tintTopLeft) || 17578,
        )
        .setDepth(20)
        .setVisible(false);
    ...
    const floorLineFrame = this._scene.textures.getFrame(
      "GJ_WebSheet",
      "floorLine_01_001.png",
    );
    const floorLineScale =
      screenWidth / (floorLineFrame ? floorLineFrame.width : 888);
  ...
  updateGroundTiles(cameraY = 0) {
    ...
    if (this._flyVisualOnly && this._flyCeilingY !== null) {
      ...
      leftTileIndex =
        ...
        (targetGroundY - this._groundStartScreenY) * groundTarget;
      rightTileIndex =
        this._ceilingStartScreenY +
        (targetCeilingY - this._ceilingStartScreenY) * groundTarget;
    ...
    for (let i = 0; i < this._groundTiles.length; i++) {
      ...
      ceilingTile.setVisible(
        (this._flyGroundActive && this._groundTargetValue > 0) || ceilingActive,
      );
    }
  ...
  resetGroundTiles(cameraX) {
    ...
    this._maxGroundWorldX =
      cameraX + (this._groundTiles.length - 1) * tileWidth;
  ...
  stepGroundAnimation(deltaTime) {
    ...
    this._groundTargetValue =
      ...
      (this._groundAnimTo - this._groundAnimFrom) * progress;
    if (progress >= 1) {
      this._groundAnimating = false;
  ...
  _applyVisualProps(scene, sprite, frameName, objectData, colorData = null) {
    ...
    let { dx: offsetX, dy: offsetY } = (function (scene, frameName) {
    ...
    let totalRotation =
      (sprite.getData("gjBaseRotationDeg") || 0) + objectData.rot;
    if (totalRotation !== 0) {
  ...
  _spawnLevelObjects(_0x35f1ae) {
    ...
    for (let levelObj of _0x35f1ae) {
      ...
      if (objectDef && objectDef.randomFrames) {
        frameName =
          objectDef.randomFrames[
            Math.floor(Math.random() * objectDef.randomFrames.length)
          ];
      }
      if (frameName) {
        ...
        let _0xOrbGlow = null;
        if (objectDef.glow) {
          _0xOrbGlow = this._addGlowSprite(
            scene,
            spriteWorldX,
            baseY,
            frameName,
        ...
        if (sprite) {
          ...
          console.warn(
            "No sprite found for object ID " +
              levelObj.id +
              " frame=" +
              frameName +
              " type=" +
              (objectDef ? objectDef.type : "null"),
          );
        }
        if (
          objectDef &&
          (objectDef.type === solidType || objectDef.type === hazardType)
        ) {
          let _0x47077e = frameName.replace("_001.png", "_2_001.png");
          let overlaySprite = R(scene, _0x47077e)
            ? L(scene, spriteWorldX, baseY, _0x47077e)
            : null;
          if (overlaySprite) {
        ...
        if (objectDef.children) {
          for (let childDef of objectDef.children) {
            ...
            if (
              childDef.localDx !== undefined ||
              childDef.localDy !== undefined
            ) {
              ...
              _0x3b4e8c =
                _0x38902b * Math.cos(_0x3e62f2) -
                _0x256a8e * Math.sin(_0x3e62f2);
              _0x172131 =
                _0x38902b * Math.sin(_0x3e62f2) +
                _0x256a8e * Math.cos(_0x3e62f2);
            }
            const _childWorldX = worldX + _0x3b4e8c;
            const _childBaseY = baseY + _0x172131;
            let childSprite = L(
              scene,
              spriteWorldX + _0x3b4e8c,
              baseY + _0x172131,
              childDef.frame,
            );
            if (childSprite) {
        ...
        console.warn(
          "Object ID " +
            levelObj.id +
            " has no definition in allObjects at x=" +
            worldX +
            " y=" +
            worldY,
        );
      ...
      if (objectDef && objectDef.portalParticle && frameName) {
        ...
        const _0x388526 = {
          getRandomPoint: (_0x4ad804) => {
            ...
            _0x4ad804.x =
              _rx * Math.cos(_portalRot) - _ry * Math.sin(_portalRot);
            _0x4ad804.y =
              _rx * Math.sin(_portalRot) + _ry * Math.cos(_portalRot);
            return _0x4ad804;
          },
        };
        const _0x100649 = 20;
        let _0x1bed6b = scene.add.particles(
          _0x5926ad,
          _0x1ebc69,
          "GJ_WebSheet",
          {
            ...
            emitCallback: (_0x157c59) => {
              ...
              let _0x42124a =
                Math.sqrt(_0x30a90b * _0x30a90b + _0x3e98bf * _0x3e98bf) || 1;
      ...
      if (objectDef) {
        ...
        if (
          ...
          if (levelObj.id === 111) {
            console.log(
              "res - _0x5bcd81: " + _0x5bcd81 + ", _0x25452a: " + _0x25452a,
            );
          }
          if (!_0x25452a) {
            console.warn(
              "unknown portal sub-type: id=" +
                levelObj.id +
                " sub=" +
                objectDef.sub,
            );
    ...
    if (_0x443c50.size > 0) {
      console.warn(
        "" +
          _0x443c50.size +
          " unique object IDs had no definition in allObjects:",
        [..._0x443c50].join(", "),
      );
    }
    let colTypeCounts = {};
    ...
    this._colorTriggers.sort(
      (_0x359c7f, _0x28dd8b) => _0x359c7f.x - _0x28dd8b.x,
    );
    this._enterEffectTriggers.sort(
      (_0x3e43f2, _0x5e3d9a) => _0x3e43f2.x - _0x5e3d9a.x,
    );
  ...
  createEndPortal(_0x41fbdb) {
    ...
    for (let _0x2a327c = 0; _0x2a327c < _0x46064b; _0x2a327c++) {
      const _0xacf7ef = _0x41fbdb.add
        .image(
          0,
          (_0x2a327c - Math.floor(_0x46064b / 2)) * a,
          "GJ_WebSheet",
          "square_02_001.png",
        )
        .setAngle(-90);
    ...
    this._endPortalShine = _0x41fbdb.add.image(
      _0x3b56d4 - 58,
      _0x1c3aea,
      "GJ_WebSheet",
      "gradientBar.png",
    ...
    const _0x3e25a9 =
      ((_0x400605 = _0x41fbdb.textures.getFrame(
        "GJ_WebSheet",
        "gradientBar.png",
      )) == null
        ? undefined
    ...
    this._endPortalEmitter = _0x41fbdb.add.particles(
      _0x58cedb,
      _0x1c3aea,
      "GJ_WebSheet",
      {
        ...
        emitCallback: (_0x2daff4) => {
          ...
          const _0x3c5c52 =
            Math.sqrt(_0x5e30d8 * _0x5e30d8 + _0x17ba71 * _0x17ba71) || 1;
  ...
  _addToSection(sliderWidth) {
    ...
    const _0x14d5f7 =
      sliderWidth._eeLayer !== undefined ? sliderWidth._eeLayer : 1;
    if (_0x14d5f7 === 2) {
  ...
  updateVisibility(_0xa5f1e1) {
    ...
    const sliderHeight = Math.min(
      _0x1dce22,
      Math.floor((_0xa5f1e1 + screenWidth + 200) / 400),
    );
    ...
    if (_0x1800fc < 0) {
      for (let _0x47dbe1 = 0; _0x47dbe1 <= _0x1dce22; _0x47dbe1++) {
        this._setSectionVisible(
          _0x47dbe1,
          _0x47dbe1 >= particleScale && _0x47dbe1 <= sliderHeight,
        );
      }
    ...
    if (particleScale !== _0x1800fc || sliderHeight !== _0xc31046) {
      if (particleScale > _0x1800fc) {
        for (
          let _0x7da5df = _0x1800fc;
          _0x7da5df <= Math.min(particleScale - 1, _0xc31046);
          _0x7da5df++
        ) {
      ...
      if (sliderHeight < _0xc31046) {
        for (
          let _0x5b2d47 = Math.max(sliderHeight + 1, _0x1800fc);
          _0x5b2d47 <= _0xc31046;
          _0x5b2d47++
        ) {
      ...
      if (particleScale < _0x1800fc) {
        for (
          let _0x3caab6 = particleScale;
          _0x3caab6 <= Math.min(_0x1800fc - 1, sliderHeight);
          _0x3caab6++
        ) {
      ...
      if (sliderHeight > _0xc31046) {
        for (
          let _0x347412 = Math.max(_0xc31046 + 1, particleScale);
          _0x347412 <= sliderHeight;
          _0x347412++
        ) {
  ...
  stepMoveTriggers(dt) {
    ...
    while (i < this._activeMoveTweens.length) {
      ...
      const curSample = Easing.sample(
        trig.easingType,
        trig.easingRate,
        progress,
      );
      const prevSample = Easing.sample(
        trig.easingType,
        trig.easingRate,
        prevProgress,
      );
  ...
  applyEnterEffects(_0x2f36ed) {
    ...
    const _0x2d8f53 = Math.max(
      0,
      Math.floor((_0x29a51b - _0xa24372) / _0x221c93),
    );
    const _0x2b19db = Math.min(
      this._sections.length - 1,
      Math.floor((_0x548004 + _0xa24372) / _0x221c93),
    );
    for (let _0x1bd44f = _0x2d8f53; _0x1bd44f <= _0x2b19db; _0x1bd44f++) {
      ...
      const _0x8f9d56 =
        _0x20a3bb >= _0x29a51b + _0xa24372 &&
        _0x20a3bb + _0x221c93 <= _0x548004 - _0xa24372;
      for (let _0x54aba7 = 0; _0x54aba7 < _0x2cff29.length; _0x54aba7++) {
        ...
        let _0x289aa2;
        _0x289aa2 = _0x1b2883
          ? Math.max(0, Math.min(1, (_0x548004 - _0xeded99) / _0xa24372))
          : Math.max(0, Math.min(1, (_0xeded99 - _0x29a51b) / _0xa24372));
        if (_0x289aa2 >= 1) {
          if (effectSprite._eeActive) {
            effectSprite._eeActive = false;
            effectSprite.y = effectSprite._eeBaseY;
...
class cs {
  constructor(
    ...
    this._posInit = false;
    this._active = false;
    const graphicsSettings = window.performanceOptimizer
      ? window.performanceOptimizer.getGraphicsSettings()
      : {
          enableGlow: true,
          blendMode: Phaser.BlendModes.ADD,
        };
  ...
  update(_0x2acf4c) {
    ...
    if (this._active && this._pts.length < this._maxPoints) {
      ...
      if (_0x89a79d > 0) {
        ...
        if (this._maxSeg > 0 && Math.sqrt(_0x1f9fea) > this._maxSeg) {
          ...
          if (
            _0x14c0c1 * _0x14c0c1 + _0x2d01f0 * _0x2d01f0 <
            this._minSegSq * 2
          ) {
            _0x3d12ca = false;
          }
    ...
    if (!(_0x49dac5 < 2)) {
      for (let _0x27c164 = 0; _0x27c164 < _0x49dac5 - 1; _0x27c164++) {
        ...
        const _0x1c4c9d =
          (_0x398b7b.state + _0x3b4326.state) * 0.5 * this._opacity;
...
class WaveTrail {
  ...
  _buildEdges(pts, halfW) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (i === 0) {
        ...
        upper[i] = {
          x: px + segNx[i - 1] * halfW,
          y: py + segNy[i - 1] * halfW,
        };
        lower[i] = {
          x: px - segNx[i - 1] * halfW,
          y: py - segNy[i - 1] * halfW,
  ...
  _drawRibbon(gfx, pts, halfW, color, baseAlpha) {
    ...
    for (let i = 0; i < n - 1; i++) {
      const alpha =
        Math.max(0, (1 - pts[i].age + (1 - pts[i + 1].age)) * 0.5) * baseAlpha;
...
class ps {
  ...
  _createSprites() {
    ...
    this._playerGlowLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${window.currentPlayer}_glow_001.png`,
      9,
      false,
    );
    this._playerSpriteLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${window.currentPlayer}_001.png`,
      10,
      true,
    );
    this._playerOverlayLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${window.currentPlayer}_2_001.png`,
      8,
      true,
    );
    this._playerExtraLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${window.currentPlayer}_extra_001.png`,
      12,
      true,
    );
    ...
    if (this._playerSpriteLayer) {
      ...
      let _0x3aecd9 = spriteY.add.rectangle(
        particleY,
        spriteX,
        g,
        g,
    ...
    this._shipGlowLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${window.currentShip}_glow_001.png`,
      9,
      false,
    );
    this._shipSpriteLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${window.currentShip}_001.png`,
      10,
      false,
    );
    this._shipOverlayLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${window.currentShip}_2_001.png`,
      8,
      false,
    );
    this._shipExtraLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${window.currentShip}_extra_001.png`,
      12,
      false,
    );
    ...
    if (this._shipSpriteLayer) {
      ...
      let _0x100643 = spriteY.add.polygon(
        particleY,
        spriteX,
        [
          {
    ...
    this._ballGlowLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${window.currentBall}_glow_001.png`,
      9,
      false,
    );
    this._ballSpriteLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${window.currentBall}_001.png`,
      10,
      false,
    );
    this._ballOverlayLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${window.currentBall}_2_001.png`,
      8,
      false,
    );
    ...
    ].filter((_0x37ad93) => !!_0x37ad93);
    this._waveLayers = [
      this._waveSpriteLayer,
      this._waveOverlayLayer,
      this._waveExtraLayer,
      this._waveGlowLayer,
    ].filter((_0x37ad93) => !!_0x37ad93);
    const _spiderBase = `${window.currentSpider}_01`;
    this._spiderSpriteLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${_spiderBase}_001.png`,
      10,
      false,
    );
    this._spiderGlowLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${_spiderBase}_glow_001.png`,
      9,
      false,
    );
    this._spiderOverlayLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${_spiderBase}_2_001.png`,
      8,
      false,
    );
    this._spiderExtraLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${_spiderBase}_extra_001.png`,
      12,
      false,
    );
    if (this._spiderSpriteLayer)
      this._spiderSpriteLayer.sprite.setTint(window.mainColor);
    if (this._spiderOverlayLayer)
      this._spiderOverlayLayer.sprite.setTint(window.secondaryColor);
    ...
    this._spiderLayers = [
      this._spiderSpriteLayer,
      this._spiderGlowLayer,
      this._spiderOverlayLayer,
      this._spiderExtraLayer,
    ].filter((x) => !!x);
    this._birdSpriteLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${window.currentBird}_001.png`,
      10,
      false,
    );
    this._birdGlowLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${window.currentBird}_2_001.png`,
      9,
      false,
    );
    this._birdOverlayLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${window.currentBird}_3_001.png`,
      8,
      false,
    );
    this._birdExtraLayer = ds(
      spriteY,
      particleY,
      spriteX,
      `${window.currentBird}_extra_001.png`,
      12,
      false,
    );
    ...
    this._dashAnimationSprite = spriteY.add.image(
      particleY,
      spriteX,
      "GJ_GameSheetGlow",
      "playerDash2_001.png",
  ...
  _updateParticles(_0xc43238, _0x52b718, _0x5af874) {
    ...
    this._particleEmitter.particleY =
      _0x519d38 +
      (this.p.gravityFlipped
        ? -26 + (this.p.isUfo ? -5 : 0)
        : 26 + (this.p.isUfo ? 5 : 0));
    const _0x4436ac =
      this.p.onGround && !this.p.isFlying && !this.p.isWave && !this.p.isSpider;
    ...
    }
    {
      ...
      const _0x2baeac =
        (this.p.isWave ? 4 : this.p.isUfo ? 5 : 18) *
        (this.p.gravityFlipped ? -1 : 1);
      const _0x75c380 =
        _0x119eb7 + _0x216018 * _0xe76a85 - _0x2baeac * _0x26ec65;
      const _0x2b31d7 =
        _0x519d38 + _0x216018 * _0x26ec65 + _0x2baeac * _0xe76a85;
      ...
      this._streak.setPosition(
        this.p.isWave ? _0x75c380 : this.p.isUfo ? _0x75c380 : _0x75c380 + 8,
        _0x2b31d7,
      );
    ...
    const _0x169e30 =
      (this.p.isFlying && this.p.upKeyDown) ||
      (this.p.isUfo && this.p.isJumping);
    ...
    const _0x2e5643 =
      _0xc43238 + this._scene._getMirrorXOffset(_0x119eb7 - _0xc43238);
    this._shipDragEmitter.x = _0x2e5643;
    this._shipDragEmitter.particleY = this.p.gravityFlipped
      ? b(this.p.y) + _0x52b718 + 10
      : b(this.p.y) + _0x52b718 + 30;
    this._shipDragEmitter.setAngle(
      this.p.mirrored
        ? {
            min: 245,
            max: 335,
    ...
    this._shipDragEmitter.setScale(
      this.p.gravityFlipped ? { x: -1, y: 1 } : { x: 1, y: 1 },
    );
    const _0x2ac9d0 =
      this.p.isFlying &&
      this.p.onGround &&
      (this.p.gravityFlipped ? this.p.onCeiling : !this.p.onCeiling);
  ...
  setCubeVisible(_0x411813) {
    ...
    if (this._playerGlowLayer) {
      this._playerGlowLayer.sprite.setVisible(
        _0x411813 && this._playerGlowLayer.sprite._glowEnabled,
      );
    }
    if (this._playerOverlayLayer) {
  ...
  setShipVisible(_0x1c5620) {
    ...
    if (this._shipGlowLayer) {
      this._shipGlowLayer.sprite.setVisible(
        _0x1c5620 && this._shipGlowLayer.sprite._glowEnabled,
      );
    }
    if (this._shipOverlayLayer) {
  ...
  setBallVisible(_0x5685cf) {
    ...
    if (this._ballGlowLayer) {
      this._ballGlowLayer.sprite.setVisible(
        _0x5685cf && this._ballGlowLayer.sprite._glowEnabled,
      );
    }
    if (this._ballOverlayLayer) {
  ...
  setWaveVisible(_0x2d078b) {
    ...
    if (this._waveGlowLayer) {
      this._waveGlowLayer.sprite.setVisible(
        _0x2d078b && this._waveGlowLayer.sprite._glowEnabled,
      );
    }
  }
  setSpiderVisible(v) {
  ...
  syncSprites(cameraX, cameraY, _0x3afedf, mirrorOffset) {
    ...
    if (this.p.isFlying || this.p.isUfo) {
      ...
      if (this.p.isFlying) {
        for (const layer of this._shipLayers) {
          if (layer) {
            ...
            layer.sprite.y =
              _0x1a433c + _0x185f91 + (this.p.gravityFlipped ? -20 : 0);
            layer.sprite.rotation = this.p.mirrored
              ? -playerRotation
              : playerRotation;
      ...
      if (this.p.isUfo && !this.p.isDead) {
        for (const layer of this._birdLayers) {
          if (layer) {
            ...
            layer.sprite.x = _0x7f0705 + _0x1b1d28;
            layer.sprite.y =
              _0x1a433c + _0x185f91 + (this.p.gravityFlipped ? -15 : 5);
            layer.sprite.rotation = this.p.mirrored
              ? -playerRotation
              : playerRotation;
            const _miniS = this.p.isMini ? 0.6 : 1;
      ...
      for (const playerLayerItem of this._playerLayers) {
        if (playerLayerItem) {
          ...
          playerLayerItem.sprite.y =
            _0x1a433c +
            _0x3011c9 +
            (this.p.isMini ? 8 : 0) +
            (this.p.gravityFlipped ? -20 : 0);
          playerLayerItem.sprite.rotation = this.p.mirrored
            ? -playerRotation
            : playerRotation;
          const _miniS = this.p.isMini ? 0.6 : 1;
          const _shipCubeS = _miniS * 0.55;
          playerLayerItem.sprite.scaleY = this.p.gravityFlipped
            ? -_shipCubeS
            : _shipCubeS;
          playerLayerItem.sprite.scaleX = this.p.mirrored
            ? -_shipCubeS
            : _shipCubeS;
        }
      }
      if (_ufoMode) {
        const _ufoTilt = Math.max(
          -0.05,
          Math.min(0.05, -(this.p.y - this.p.lastY) * 0.008),
        );
        for (const layer of this._birdLayers) {
          if (layer) {
      ...
      for (const playerLayer of this._allLayers) {
        if (playerLayer) {
          ...
          playerLayer.sprite.rotation = isBallLayer
            ? playerRotation
            : this.p.mirrored
              ? -playerRotation
              : playerRotation;
    ...
    if (this._dashAnimationSprite && this._dashAnimationSprite.visible) {
      ...
      const _miniS = this.p.isMini ? 0.6 : 1;
      this._dashAnimationSprite.scaleY = this.p.gravityFlipped
        ? -_miniS
        : _miniS;
  ...
  enterShipMode(_0xeb37c6 = null) {
    ...
    if (_0xeb37c6) {
      _0x17d728 =
        _0xeb37c6.portalY !== undefined ? _0xeb37c6.portalY : _0xeb37c6.y;
    }
  ...
  enterBallMode(_0x36bb3d = null) {
    ...
    if (_0x36bb3d) {
      _0x18df19 =
        _0x36bb3d.portalY !== undefined ? _0x36bb3d.portalY : _0x36bb3d.y;
    }
  ...
  enterWaveMode(_0x5a10cc = null) {
    ...
    if (_0x5a10cc) {
      _0x38b484 =
        _0x5a10cc.portalY !== undefined ? _0x5a10cc.portalY : _0x5a10cc.y;
    }
  ...
  hitGround() {
    ...
    if (_0x4a38a5 && !this.p.isFlying && !this.p.isWave && !this.p.isSpider) {
      ...
      const _0x17e0bb = this.p.gravityFlipped
        ? b(this.p.y) - 30
        : b(this.p.y) + 30;
  ...
  killPlayer() {
    ...
    const _0x3f0446 = _0x3f4b84._getMirrorXOffset(
      _0x3f4b84._playerWorldX - _0x3f4b84._cameraX,
    );
    const _0x53ac5b = b(this.p.y) + this._lastCameraY;
    const _0x281e43 = 0.9;
    _0x3f4b84.add
      .particles(_0x3f0446, _0x53ac5b, "GJ_WebSheet", {
        frame: "square.png",
        speed: {
          min: 200,
          max: 800,
  ...
  _createExplosionPieces(_0x49be85, _0x13b56e, _0x349a09) {
    ...
    for (const _0x1f09e3 of _0x5c571a) {
      ...
      _0x26dcbd.draw(
        _0x53102a,
        sliderBar / 2 + (_0x53102a.x - _0x49be85),
        sliderBar / 2 + (_0x53102a.y - _0x13b56e),
      );
    }
    ...
    for (let _0x3f4d44 = 0; _0x3f4d44 < _0x28c600 - 1; _0x3f4d44++) {
      const _0x5b2c12 = Math.round(
        _0x5e8097 * (0.55 + Math.random() * _0x4dd9c4 * 2),
      );
    ...
    for (let _0x325ce1 = 0; _0x325ce1 < _0x247253 - 1; _0x325ce1++) {
      const _0x37f0ad = Math.round(
        _0x5af9d3 * (0.55 + Math.random() * _0x4dd9c4 * 2),
      );
    ...
    this._explosionContainer = _0x44acaf.add
      .container(_0x49be85, _0x13b56e)
      .setDepth(16);
    let _0x156c8b = 0;
    for (let _0x4cd06e = 0; _0x4cd06e < _0x28c600; _0x4cd06e++) {
      ...
      for (let _0x5b14cf = 0; _0x5b14cf < _0x247253; _0x5b14cf++) {
        ...
        const _0x159cfa = {
          ...
          particle: _0x298d34,
          xVel:
            (_0x422587 + (Math.random() * 2 - 1) * _0x1e87b0) *
            (this.p.mirrored ? -1 : 1),
  ...
  _playPortalShine(_0x49e81f) {
    ...
    for (let _0x34fd8c = 0; _0x34fd8c < 2; _0x34fd8c++) {
      ...
      const pieceSize = _0x4ed8ff.add.image(
        _0xf31b0d,
        _0x3824c0,
        _0x4bfe30.atlas,
        _0x4bfe30.frame,
  ...
  _checkSnapJump(_0x1f801b) {
    ...
    if (_0x2b806a && _0x2b806a !== _0x1f801b && _0x2b806a.type === solidType) {
      ...
      for (const _0x31d5e4 of _0x483058) {
        if (
          Math.abs(_0x5de781 - (_0x34ef27 + _0x31d5e4.dx)) <= 2 &&
          Math.abs(_0x21ad88 - (_0x4652bb + _0x31d5e4.dy * _0x1b1831)) <= 2
        ) {
          _0x372d4e = true;
          break;
        }
      ...
      if (_0x372d4e) {
        ...
        let _0x5f2847;
        _0x5f2847 =
          Math.abs(_0x4ca454 - _0x48aba3) <= 2
            ? _0x4ca454
            : _0x4ca454 > _0x48aba3
              ? _0x48aba3 + 2
              : _0x48aba3 - 2;
  ...
  updateRotateAction(_0x98044d) {
    ...
    let _0xb1cb91 = Math.min(
      this.rotateActionTime / this.rotateActionDuration,
      1,
    );
    this._rotation =
      this.rotateActionStart + this.rotateActionTotal * _0xb1cb91;
  }
  ...
  updateJump(_0x3d1c6f) {
    ...
    if (this.p.isFlying) {
      ...
      if (this.playerIsFalling()) {
        let _0x47ed2a;
        _0x47ed2a = this.p.gravityFlipped
  ...
  _updateBallJump(_0x2fe319) {
    ...
    if (this.playerIsFalling()) {
      const _0x1439be = this.p.gravityFlipped
        ? this.p.yVelocity > p * 2
        : this.p.yVelocity < -(p * 2);
      if (_0x1439be) {
        this.p.onGround = false;
  ...
  _updateWaveJump() {
    ...
    this._rotation =
      _0x312a7f === 0 ? 0 : _0x312a7f > 0 ? -_waveAngle : _waveAngle;
  }
  _updateUfoJump(_dt) {
    ...
    if (this.p.upKeyPressed) {
      ...
      this.p.isJumping = true;
      try {
        this._flyParticle2Emitter.explode(
          6,
          this._scene._playerWorldX,
          b(this.p.y) + (this.p.gravityFlipped ? -18 : 18),
        );
      } catch (e) {}
    }
    if (!this.p.wasBoosted) {
  ...
  _updateSpiderJump(dt) {
    ...
    if (this.p.upKeyPressed && this.p.canJump) {
      ...
      if (!this.p.gravityFlipped) {
        ...
        const nearbyObjects =
          this._gameLayer.getNearbySectionObjects(playerWorldX);
  ...
  checkCollisions(_0x2f5078) {
    ...
    const _0x11ee2f =
      this.p.isFlying || this.p.isWave || this.p.isUfo ? 12 : 20;
    this.p.collideTop = 0;
    ...
    for (let gameObj of _0x198534) {
      ...
      if (
        !(pieceWidth + _broadSize <= rotatedLeft) &&
        !(pieceWidth - _broadSize >= rotatedRight) &&
        !(_0x8e0d28 + _broadSize <= rotatedTop) &&
        !(_0x8e0d28 - _broadSize >= rotatedBottom)
      ) {
        ...
        if (_colType === "portal_fly") {
          ...
          const _needsClick = _isDash
            ? this.p.upKeyDown
            : this.p.queuedHold && this.p.upKeyDown;
          if (!gameObj.activated && _needsClick) {
            if (_isDash) {
              ...
              if (gameObj._dashHoldTicks < 2) {
                ...
                if (_clampedAngle > 270) {
                  _clampedAngle -= 360;
                }
                if (_clampedAngle >= 90 && _clampedAngle <= 270) {
                  _clampedAngle = 180 - _clampedAngle;
                }
                _clampedAngle = Math.max(-70, Math.min(70, _clampedAngle));
                const _dashRad = (_clampedAngle * Math.PI) / 180;
                const _dashSpeed = 18;
                const _dashVelY =
                  Math.sin(_dashRad) * _dashSpeed * this.flipMod();
                if (_orbId === 1751) {
                  this.flipGravity(!this.p.gravityFlipped);
                }
          ...
          if (!gameObj.activated) {
            gameObj.activated = true;
            try {
              const _coinSpr = this._gameLayer._coinSprites.find(
                (s) =>
                  s &&
                  s.active &&
                  Math.abs(s._coinWorldX - gameObj.x) < 2 &&
                  Math.abs(s._coinWorldY - gameObj.y) < 2,
              );
          ...
          const _0x55559d = 9;
          const _0x3c1654 =
            pieceWidth + _0x55559d > left &&
            pieceWidth - _0x55559d < right &&
            _0x8e0d28 + _0x55559d > top &&
            _0x8e0d28 - _0x55559d < bottom;
          const _0xLandBot =
            (this.p.yVelocity <= 0 || this.p.onGround) &&
            (_0x146a97 >= bottom || _0x869e42 >= bottom);
          const _0xLandTop =
            (this.p.yVelocity >= 0 || this.p.onGround) &&
            (_0x3e7199 <= top || _0x135a9d <= top);
          ...
          if (
            pieceWidth + playerSize - 5 > left &&
            pieceWidth - playerSize + 5 < right
          ) {
            if (
              !this.p.gravityFlipped &&
              (_0x146a97 >= bottom || _0x869e42 >= bottom) &&
    ...
    if (this.p.isFlying || this.p.isWave || this.p.isUfo || this.p.isSpider) {
      ...
      const _0xdc296 =
        _0x496456 !== null && this.p.y >= _0x496456 - _effectiveSize;
  ...
  drawHitboxes(graphics, camX, camY) {
    ...
    for (let nearObject of nearbyObjects) {
      ...
      if (nearObject.type === hazardType) {
        ...
        nearObject.type === "portal_fly" ||
        nearObject.type === "portal_cube" ||
        nearObject.type === "portal_ball" ||
        nearObject.type === portalWaveType ||
        nearObject.type === portalUfoType
      ) {
        hitboxColor = 4491519;
      } else if (
        nearObject.type === "portal_gravity_down" ||
        nearObject.type === "portal_gravity_up"
      ) {
        hitboxColor = 16776960;
      } else if (
        nearObject.type === "portal_mirror_on" ||
        nearObject.type === "portal_mirror_off"
      ) {
        hitboxColor = 16744448;
      } else if (
        nearObject.type === "portal_mini_on" ||
        nearObject.type === "portal_mini_off"
      ) {
    ...
    graphics.strokeCircle(
      centerX - playerSize + hitboxsize / 2,
      _0x1e788a - playerSize + hitboxsize / 2,
      hitboxsize / 2,
    );
    // ----
    // inner hitbox (blue)
  ...
  playEndAnimation(_0x24408e, _0x281588, _0x54bbf4) {
    ...
    const _0x11b580 = [
      ...
      .filter((_0x3e9c62) => _0x3e9c62 && _0x3e9c62.sprite.visible)
      .map((_0x5cedeb) => _0x5cedeb.sprite);
    this._particleEmitter.stop();
    this._flyParticleEmitter.stop();
    this._flyParticle2Emitter.stop();
    this._shipDragEmitter.stop();
    ...
    const _0x3fc5a5 = _0x11b580.map((_0x5c0e81) => {
      let _0x5cbb0a = 0;
      if (_0x154798) {
        const _0xff16eb = _0x3793a4.some(
          (_0x40ef1e) => _0x40ef1e && _0x40ef1e.sprite === _0x5c0e81,
        );
        const _0x4fdb53 = _0xbd676f.some(
          (_0x4ef5b5) => _0x4ef5b5 && _0x4ef5b5.sprite === _0x5c0e81,
        );
        if (_0xff16eb) {
          _0x5cbb0a = 10;
        } else if (_0x4fdb53) {
    ...
    _0x3729ef.tweens.add({
      ...
      onUpdate: () => {
        ...
        const _0x2478d6 =
          (1 - spriteWidth) ** 3 * _0x1295ea +
          (1 - spriteWidth) ** 2 * 3 * spriteWidth * _0x1295ea +
          (1 - spriteWidth) * 3 * spriteWidth ** 2 * _0x1f2e19 +
          spriteWidth ** 3 * _0x457676;
        const _0x148e69 =
          (1 - spriteWidth) ** 3 * _0x47ae60 +
          (1 - spriteWidth) ** 2 * 3 * spriteWidth * _0x47ae60 +
          (1 - spriteWidth) * 3 * spriteWidth ** 2 * _0x8bc9f4 +
          spriteWidth ** 3 * _0x3ade39;
...
class vs {
  ...
  step(_0x4559d6) {
    ...
    let _0xe145bf =
      this.duration > 0 ? Math.min(this.elapsed / this.duration, 1) : 1;
    if (_0xe145bf >= 1) {
      this.current = {
        ...this.to,
      };
...
class ys {
  ...
  _playOnlineBuffer(audioBuffer, startOffset = 0) {
    ...
    if (this._onlineSource) {
      try {
        this._onlineSource.stop();
      } catch (e) {}
      try {
        this._onlineSource.disconnect();
      } catch (e) {}
    ...
    const dest =
      soundMgr.masterVolumeNode || soundMgr.destination || ctx.destination;
    gainNode.connect(dest);
    const safeOffset = Math.max(
      0,
      Math.min(startOffset, audioBuffer.duration - 0.01),
    );
    ...
    const musicObj = {
      ...
      pause: () => {
        ...
        _pauseOffset =
          (ctx.currentTime - _startedAt + _pauseOffset) % audioBuffer.duration;
  ...
  playEffect(_0x344122, _0x20f8e7 = {}) {
    if (
      this._scene.sound.context &&
      this._scene.cache.audio.exists(_0x344122)
    ) {
  ...
  _ensureCorrectMusicMode() {
    ...
    const isPracticeMode =
      this._scene._practicedMode && this._scene._practicedMode.practiceMode;
    const expectedSongKey = isPracticeMode
      ? "StayInsideMe"
  ...
  update(_0x34aeef) {
    ...
    if (
      ...
      this._meterValue < this._lastAudio * 1.1 ||
      (this._meterValue < this._lastPeak * 0.95 &&
        this._lastAudio > this._lastPeak * 0.2)
    ) {
...
class xs extends Phaser.Scene {
  ...
  create() {
    ...
    this._glitterEmitter = this.add.particles(0, 0, "GJ_WebSheet", {
      ...
      emitCallback: (_0x3c2a3e) => {
        _0x3c2a3e.x =
          this._glitterCenterX + (Math.random() * 2 - 1) * (screenWidth / 1.8);
    ...
    this._bestPercent = 0;
    this._lastPercent = 0;
    this._practiceBestPercent = parseFloat(
      localStorage.getItem(
        "practiceBestPercent_" + (window.currentlevel[2] || "level_1"),
      ) || "0",
    );
    this._endPortalGameY = 240;
    ...
    for (let _0xfeaf5c = 0; _0xfeaf5c < _0x4fc67f.length; _0xfeaf5c++) {
      ...
      const _0x1d293f = this.add
        .image(0, 0, "GJ_WebSheet", _0x1ce2a6.key + ".png")
        .setScrollFactor(0)
        .setDepth(30)
        .setScale(_0x6bf69f)
        .setInteractive();
      this._makeBouncyButton(
        _0x1d293f,
        _0x6bf69f,
        () => window.open(_0x1ce2a6.url, "_blank"),
        () => this._menuActive,
      );
    ...
    this._menuUpdateLogBtn = this.add
      .image(screenWidth - 30 - 50, 33, "GJ_WebSheet", "GJ_infoIcon_001.png")
      .setScrollFactor(0)
      .setDepth(30)
      .setScale(0.64)
      .setAlpha(0.8)
    ...
    // creator stuff
    this._creatorBtn = this.add
      .image(0, 0, "GJ_GameSheet04", "GJ_creatorBtn_001.png")
      .setScrollFactor(0)
      .setDepth(30)
      .setInteractive()
      .setScale(1);
    ...
    this._openCreatorMenu = () => {
      ...
      const blocker = this.add
        .zone(sw / 2, sh / 2, sw, sh)
        .setScrollFactor(0)
        .setDepth(101)
        .setInteractive();

      const cornerTL = this.add
        .image(0, 0, "GJ_GameSheet03", "GJ_sideArt_001.png")
        .setScrollFactor(0)
        .setDepth(103)
        .setOrigin(0, 0)
        .setFlipX(true)
      ...
      const cornerBL = this.add
        .image(0, sh, "GJ_GameSheet03", "GJ_sideArt_001.png")
        .setScrollFactor(0)
        .setDepth(103)
        .setOrigin(0, 1)
        .setFlipX(true)
      ...
      const backBtn = this.add
        .image(50, 48, "GJ_GameSheet03", "GJ_arrow_03_001.png")
        .setScrollFactor(0)
        .setDepth(104)
        .setFlipX(true)
        .setFlipY(true)
      ...
      menuButtons.forEach((frame, idx) => {
        ...
        const btn = this.add
          .image(bx, by, "GJ_GameSheet04", frame)
          .setScrollFactor(0)
          .setDepth(104)
          .setScale(btnScale);
    ...
    this._openSearchMenu = () => {
      ...
      const placeholderLabel = this.add
        .bitmapText(
          sw / 2,
          inputY + inputH / 2,
          "bigFont",
        ...
        .setOrigin(0.5, 0.5)
        .setTint(0xaaddff);
      const typedLabel = this.add
        .bitmapText(sw / 2, inputY + inputH / 2, "bigFont", "", 18)
        .setScrollFactor(0)
        .setDepth(105)
        .setOrigin(0.5, 0.5)
        .setTint(0xffffff);
      ...
      const _doSearchInner = async (levelId, loadFromUrl = false) => {
        ...
        _showStatus(
          `found "${levelName}"${isCustomSong ? ` — loading song #${songIdRaw}...` : ""}`,
          "#00ff00",
        );
        if (isCustomSong) {
          ...
          try {
            ...
            if (ngText && ngText !== "-1") {
              ...
              const ngMap = {};
              for (let i = 0; i + 1 < ngParts.length; i += 2)
                ngMap[ngParts[i]] = ngParts[i + 1];
              const rawUrl = (ngMap["10"] || "").trim();
              const songUrl = decodeURIComponent(rawUrl);
              const songArtist = (ngMap["4"] || "Unknown")
                .replace(/:$/, "")
                .trim();
              const songTitle = (ngMap["2"] || `Song #${songIdRaw}`)
                .replace(/:$/, "")
                .trim();
              console.log("song url:", songUrl);
              if (songUrl) {
                _showStatus(
                  `loading "${songTitle}" by ${songArtist}...`,
                  "#00ff00",
                );
    ...
    //icon stufff
    this._iconBtn = this.add
      .image(0, 0, "GJ_GameSheet03", "GJ_garageBtn_001.png")
      .setScrollFactor(0)
      .setDepth(30)
      .setInteractive()
      .setScale(1);
    ...
    this._openIconSelector = (startTab = "icon") => {
      ...
      const titleTxt = this.add
        .bitmapText(sw / 2, 60, "goldFont", "Icon Selector", 32)
        .setOrigin(0.5, 0.5)
        .setScrollFactor(0)
        .setDepth(105);
      ...
      const cornerTR = this.add
        .image(sw, 0, "GJ_GameSheet03", "GJ_sideArt_001.png")
        .setScrollFactor(0)
        .setDepth(103)
        .setOrigin(1, 0)
        .setFlipY(false)
      ...
      const cornerBR = this.add
        .image(sw, sh, "GJ_GameSheet03", "GJ_sideArt_001.png")
        .setScrollFactor(0)
        .setDepth(103)
        .setOrigin(1, 1)
        .setFlipX(false)
      ...
      const navDot1 = this.add
        .image(
          sw / 2 - navDotSpacing / 2,
          navDotY,
          "GJ_GameSheet03",
          "gj_navDotBtn_on_001.png",
        )
        .setScrollFactor(0)
        .setDepth(104)
        .setScale(0.75);
      const navDot2 = this.add
        .image(
          sw / 2 + navDotSpacing / 2,
          navDotY,
          "GJ_GameSheet03",
          "gj_navDotBtn_off_001.png",
        )
        .setScrollFactor(0)
        .setDepth(104)
        .setScale(0.75);
      const navDot3 = this.add
        .image(
          sw / 2 + navDotSpacing / 2,
          navDotY,
          "GJ_GameSheet03",
      ...
      const _updateNavDots = (page, tab) => {
        ...
        if (isShip || isIcon) {
          ...
          for (let i = 0; i < totalDotsToShow; i++) {
            dots[i]
              .setPosition(startX + i * navDotSpacing, navDotY)
              .setVisible(true);
            dots[i].setTexture(
              "GJ_GameSheet03",
              page === i
          ...
          navDot1
            .setPosition(sw / 2 - navDotSpacing / 2, navDotY)
            .setVisible(true);
          navDot2
            .setPosition(sw / 2 + navDotSpacing / 2, navDotY)
            .setVisible(true);
          navDot1.setTexture(
            "GJ_GameSheet03",
            page === 0 ? "gj_navDotBtn_on_001.png" : "gj_navDotBtn_off_001.png",
          );
          navDot2.setTexture(
            "GJ_GameSheet03",
            page === 1 ? "gj_navDotBtn_on_001.png" : "gj_navDotBtn_off_001.png",
          );
        }
      };

      const rainbowColors = [
        0xff0000, 0xff4500, 0xff7f00, 0xffaa00, 0xffd700, 0xffff00, 0xaaff00,
        0x00ff00, 0x00ff7f, 0x00ffff, 0x007fff, 0x0000ff, 0x7f00ff, 0xff00ff,
        0xff007f, 0xffffff, 0xc0c0c0, 0x808080, 0x404040, 0x000000,
      ];

      const colorBtnSize = 35;
      const colorPadding = 4;
      const colorRowWidth =
        rainbowColors.length * (colorBtnSize + colorPadding) - colorPadding;
      ...
      const colorLabel1 = this.add
        .text(sw / 2 - colorRowWidth / 2, colorRow1Y - 14, "", {
          fontSize: "11px",
          color: "#ffffff",
          fontFamily: "Arial",
        })
      ...
      const colorLabel2 = this.add
        .text(sw / 2 - colorRowWidth / 2, colorRow2Y - 14, "", {
          fontSize: "11px",
          color: "#ffffff",
          fontFamily: "Arial",
        })
      ...
      for (let ci = 0; ci < rainbowColors.length; ci++) {
        ...
        if (btn1AtlasInfo) {
          btn1 = this.add
            .image(cx, colorRow1Y, btn1AtlasInfo.atlas, btn1AtlasInfo.frame)
            .setScrollFactor(0)
            .setDepth(104)
            .setTint(rainbowColors[ci])
            .setScale(0.5)
            .setInteractive();
        } else {
          btn1 = this.add
        ...
        if (btn2AtlasInfo) {
          btn2 = this.add
            .image(cx, colorRow2Y, btn2AtlasInfo.atlas, btn2AtlasInfo.frame)
            .setScrollFactor(0)
            .setDepth(104)
            .setTint(rainbowColors[ci])
            .setScale(0.5)
        ...
        ((color, b1, b2) => {
          ...
          b1.on("pointerup", () => {
            ...
            if (this._player) {
              if (this._player._playerSpriteLayer)
                this._player._playerSpriteLayer.sprite.setTint(color);
              if (this._player._shipSpriteLayer)
                this._player._shipSpriteLayer.sprite.setTint(color);
              if (this._player._ballSpriteLayer)
                this._player._ballSpriteLayer.sprite.setTint(color);
              if (this._player._waveSpriteLayer)
                this._player._waveSpriteLayer.sprite.setTint(color);
          ...
          b2.on("pointerup", () => {
            ...
            if (this._player) {
              if (
                this._player._playerGlowLayer &&
                this._player._playerGlowLayer.sprite
              )
                this._player._playerGlowLayer.sprite.setTint(color);
              if (
                this._player._playerOverlayLayer &&
                this._player._playerOverlayLayer.sprite
              )
                this._player._playerOverlayLayer.sprite.setTint(color);
              if (
                this._player._shipGlowLayer &&
                this._player._shipGlowLayer.sprite
              )
                this._player._shipGlowLayer.sprite.setTint(color);
              if (
                this._player._shipOverlayLayer &&
                this._player._shipOverlayLayer.sprite
              )
                this._player._shipOverlayLayer.sprite.setTint(color);
              if (
                this._player._ballGlowLayer &&
                this._player._ballGlowLayer.sprite
              )
                this._player._ballGlowLayer.sprite.setTint(color);
              if (
                this._player._ballOverlayLayer &&
                this._player._ballOverlayLayer.sprite
              )
                this._player._ballOverlayLayer.sprite.setTint(color);
              if (
                this._player._waveGlowLayer &&
                this._player._waveGlowLayer.sprite
              )
                this._player._waveGlowLayer.sprite.setTint(color);
              if (
                this._player._waveOverlayLayer &&
                this._player._waveOverlayLayer.sprite
              )
                this._player._waveOverlayLayer.sprite.setTint(color);
      ...
      const selectedIconExtra = this.add
        .image(sw / 2, previewY, _iconAtlas[startTab], null)
        .setScrollFactor(0)
        .setDepth(102)
        .setVisible(false);
      const selectedIcon = this.add
        .image(sw / 2, previewY, _iconAtlas[startTab], null)
        .setScrollFactor(0)
        .setDepth(103);
      ...
      const _getPreviewFrame = (tab) => {
        ...
        const match = frames.find(
          (f) => f.replace("_001.png", "") === window[prop],
        );
      ...
      const _refreshPreview = (tab, frame) => {
        ...
        const s =
          Math.min(
            80 / (selectedIcon.width || 80),
            80 / (selectedIcon.height || 80),
          ) * 0.85;
        ...
        if (extraInfo) {
          selectedIconExtra
            .setTexture(extraInfo.atlas, extraInfo.frame)
            .setVisible(true)
            .setScale(s)
            .setTint(window.secondaryColor);
        } else {
      ...
      tabKeys.forEach((tab, i) => {
        ...
        const btn = this.add
          ...
          .setDepth(104)
          .setScale(0.75)
          .setRotation(tabRotations[tab])
          .setFlipX(tabFlipXStates[tab])
          .setFlipY(tabFlipYStates[tab])
          .setInteractive();
      ...
      const selLabel = this.add
        .image(0, 0, "GJ_GameSheet03", "GJ_select_001.png")
        .setScrollFactor(0)
        .setDepth(106)
        .setOrigin(0.5, 0.5)
        .setVisible(false);
      ...
      const nextArrow = this.add
        .image(
          containerX + containerWidth + arrowMargin,
          arrowY,
          "GJ_GameSheet03",
          "GJ_arrow_01_001.png",
        )
      ...
      const _buildGrid = (tab, page = 0) => {
        ...
        frames.forEach((frame, idx) => {
          ...
          const hitRect = this.add
            .rectangle(ix, iy, iconSize, iconSize, 0x000000, 0)
            .setScrollFactor(0)
            .setDepth(104)
            .setInteractive();
          const iconImg = this.add
            .image(ix, iy, atlas, frame)
            .setScrollFactor(0)
            .setDepth(103)
            .setTint(0xafafaf);
          const origScale =
          ...
          const extraImg = extraInfo
            ? this.add
                .image(ix, iy, extraInfo.atlas, extraInfo.frame)
                .setScrollFactor(0)
                .setDepth(102)
                .setScale(origScale)
            : null;
          ...
          ((capturedFrame, capturedImg, capturedExtra, capturedOrigScale) => {
            ...
            hitRect.on("pointerdown", () => {
              ...
              if (capturedExtra)
                capturedExtra.setScale(capturedOrigScale * 1.15);
            });
            hitRect.on("pointerup", () => {
              ...
              selLabel
                .setPosition(capturedImg.x, capturedImg.y)
                .setScale(0.75)
                .setVisible(true);

              window[prop] = capturedFrame.replace("_001.png", "");
              localStorage.setItem(
                "icon" + prop.charAt(0).toUpperCase() + prop.slice(1),
                window[prop],
              );
    ...
    if (this._iconBtn) {
      this._iconBtn.x =
        screenWidth / 2 -
        this._playBtn.width / 2 -
        50 -
        (this._iconBtn.width * this._iconBtn.scaleX) / 2;
    ...
    if (this._creatorBtn) {
      this._creatorBtn.x =
        ...
        this._playBtn.width / 2 +
        50 +
        (this._creatorBtn.width * this._creatorBtn.scaleX) / 2;
    ...
    this._leftKey = this.input.keyboard.addKey(
      Phaser.Input.Keyboard.KeyCodes.LEFT,
    );
    this._rightKey = this.input.keyboard.addKey(
    ...
    this._percentageLabel = this.add
      .bitmapText(screenWidth / 2, 20, "bigFont", "0.00%", 30)
      .setOrigin(0.5, 0.5)
      .setVisible(false)
      .setDepth(100);
    ...
    this._pauseBtn = this.add
      .image(screenWidth - 30, 30, "GJ_WebSheet", "GJ_pauseBtn_clean_001.png")
      .setScrollFactor(0)
      .setDepth(30)
      .setAlpha(75 / 255)
      .setVisible(false);
    ...
    this._restartKey = this.input.keyboard.addKey(
    ...
    this._practiceKey = this.input.keyboard.addKey(
    ...
    this._saveCheckpointKey = this.input.keyboard.addKey(
    ...
    this._saveCheckpointKey.on("down", () => {
      if (
        !this._menuActive &&
        !this._slideIn &&
        this._practicedMode.practiceMode &&
        !this._state.isDead
      ) {
        const saved = this._practicedMode.saveCheckpoint(
          this._state,
          this._playerWorldX,
          this._cameraX,
          this,
    ...
    this._deleteCheckpointKey = this.input.keyboard.addKey(
    ...
    this._levelLabel = this.add
      .bitmapText(screenWidth - 565, 30, "bigFont", window.currentlevel[1], 30)
      .setOrigin(0.5, 0.5)
      .setVisible(false);
    this._levelLabel.setScale(Math.min(1, 220 / this._levelLabel.width));

    this._leftBtn = this.add
      .image(screenWidth - 700, 30, "GJ_GameSheet03", "edit_leftBtn_001.png")
      .setScrollFactor(0)
      .setDepth(30)
      .setInteractive()
      .setVisible(false);
    this._rightBtn = this.add
      .image(screenWidth - 429, 30, "GJ_GameSheet03", "edit_leftBtn_001.png")
      .setScrollFactor(0)
      .setDepth(30)
      .setInteractive()
      .setVisible(false);
    ...
    if (this.game.registry.get("autoStartGame")) {
      ...
      if (this._practiceModeBarContainer) {
        this._practiceModeBarContainer.setVisible(
          this._practicedMode && this._practicedMode.practiceMode,
        );
      }
      this._startGame();
    }
  ...
  _parseLevelColors(levelId) {
    const LEVEL_COLORS = [
      0x0100f5, 0xf902f8, 0xf90285, 0xfa0102, 0xfa8702, 0xfcfc06, 0x03fb03,
      0x02fbfb, 0x007dff,
    ];
    let index = 0;
    if (window.allLevels) {
  ...
  _openLevelSelect() {
    ...
    const drawOverlay = (gfx, colorHex, isEveryEnd = false) => {
      ...
      const topMul = isEveryEnd ? 0.3 : 0.65;
    ...
    const tableBottom = this.add
      .image(cx, -24, "GJ_GameSheet03", "GJ_table_bottom_001.png")
      .setScrollFactor(0)
      .setDepth(152)
      .setOrigin(0.5, 0);
    const groundY = sh + 175;
    const groundId = window._groundId || "01";
    const groundFrame = this.textures.getFrame(
      "groundSquare_" + groundId + "_001.png",
    );
    ...
    for (let gi = 0; gi < numTiles; gi++) {
      const gt = this.add
        .image(gi * tileW, groundY, "groundSquare_" + groundId + "_001.png")
        .setScrollFactor(0)
        .setDepth(152)
        .setOrigin(0, 1)
        .setTint(groundTintHex(groundHex));
      staticGroundTiles.push(gt);
    }
    const floorLineFrame = this.textures.getFrame(
      "GJ_WebSheet",
      "floorLine_01_001.png",
    );
    ...
    const staticFloorLine = this.add
      .image(cx, groundY - groundTileH, "GJ_WebSheet", "floorLine_01_001.png")
      .setScrollFactor(0)
      .setDepth(153)
      .setOrigin(0.5, 0.5)
      .setScale(floorLineScale, 1)
    ...
    const cornerBR = this.add
      ...
      .setAngle(90);
    const backBtn = this.add
      .image(50, 48, "GJ_GameSheet03", "GJ_arrow_01_001.png")
      .setScrollFactor(0)
      .setDepth(154)
      .setFlipX(true)
      .setScale(1, -1)
    ...
    const infoBtn = this.add
      .image(sw - 40, 40, "GJ_GameSheet03", "GJ_infoIcon_001.png")
      .setScrollFactor(0)
      .setDepth(154)
      .setRotation(Math.PI / 2)
      .setInteractive();
    const arrowL = this.add
      .image(55, cy - 25, "GJ_GameSheet03", "navArrowBtn_001.png")
      .setScrollFactor(0)
      .setDepth(154)
      .setScale(1.1)
      .setFlipX(true)
    ...
    const arrowR = this.add
      .image(sw - 55, cy - 25, "GJ_GameSheet03", "navArrowBtn_001.png")
      .setScrollFactor(0)
      .setDepth(154)
      .setScale(1.1)
      .setFlipX(false)
    ...
    const cardX = cx;
    const cardY = cy - 100;
    const cardSlideContainer = this.add
      .container(0, 0)
      .setScrollFactor(0)
      .setDepth(152);
    const cardBounceContainer = this.add
      .container(cardX, cardY)
      .setScrollFactor(0)
      .setDepth(0);
    ...
    const buildCardContent = () => {
      ...
      const demonIcon = this.add
        .image(iconX - cardX, 0, "GJ_GameSheet03", diffFrame)
        .setScrollFactor(0)
        .setDepth(155)
        .setScale(1)
        .setOrigin(0.5, 0.5)
        .setRotation(iconRotation)
        .setFlipY(isHardDemon);
      ...
      const nameLabel = this.add
        .bitmapText(0, 0, "bigFont", lvl[1], 50)
        .setScrollFactor(0)
        .setDepth(155)
        .setOrigin(0, 0.5);
      const gap = 25;
      ...
      const groupScale = Math.min(
        1,
        maxGroupW / naturalGroupW,
        maxGroupH / naturalGroupH,
      );
    ...
    const buildBar = () => {
      ...
      barObjs.length = 0;
      const bestNormal = parseFloat(
        localStorage.getItem(
          "bestPercent_" + (window.currentlevel[2] || "level_1"),
        ) || "0",
      );
      const modeLabel = this.add
        .bitmapText(cx, barAreaY - 40, "bigFont", "Normal Mode", 30)
        .setScrollFactor(0)
        .setDepth(155)
        .setOrigin(0.5, 0.5);
      ...
      if (bestNormal > 0) {
        ...
        barFg.fillRoundedRect(
          barX0 + padding,
          barAreaY - barH2 / 2 + padding,
          fillW,
          innerH2,
          {
            tl: innerRadius,
      ...
      const pctLabel = this.add
        .bitmapText(cx, barAreaY, "bigFont", Math.round(bestNormal) + "%", 22)
        .setScrollFactor(0)
        .setDepth(156)
        .setOrigin(0.5, 0.5);
      barObjs.push(pctLabel);
      cardContainer.add(pctLabel);
      const bestPractice = parseFloat(
        localStorage.getItem(
          "practiceBestPercent_" + (window.currentlevel[2] || "level_1"),
        ) || "0",
      );
      const practBarAreaY = barAreaY + barH2 + 48;
      const practModeLabel = this.add
        .bitmapText(cx, practBarAreaY - 40, "bigFont", "Practice Mode", 30)
        .setScrollFactor(0)
        .setDepth(155)
        .setOrigin(0.5, 0.5);
      ...
      practBarBg.fillRoundedRect(
        barX0,
        practBarAreaY - barH2 / 2,
        barW2,
        barH2,
        barH2 / 2,
      );
      ...
      if (bestPractice > 0) {
        ...
        practBarFg.fillRoundedRect(
          barX0 + padding,
          practBarAreaY - barH2 / 2 + padding,
          practFillW,
          innerH2,
          {
            tl: innerRadius,
      ...
      const practPctLabel = this.add
        .bitmapText(
          cx,
          practBarAreaY,
          "bigFont",
          Math.round(bestPractice) + "%",
          22,
        )
        .setScrollFactor(0)
        .setDepth(156)
    ...
    const switchLevel = (dir) => {
      ...
      let idx = window.allLevels.findIndex(
        (l) => l[2] === window.currentlevel[2],
      );
      ...
      const scrollAnimUpdate = (time, delta) => {
        ...
        if (state === "out") {
          ...
          if (
            ...
            for (const gt of staticGroundTiles)
              gt.setTint(groundTintHex(newColors.groundHex));
            refreshDots();
            state = "in";
    ...
    this._makeBouncyButton(arrowL, 1.1, () => {
      switchLevel(-1);
    });
    this._makeBouncyButton(arrowR, 1.1, () => {
      switchLevel(1);
    });
  ...
  _buildHUD() {
    ...
    this._checkpointBtnContainer = this.add
      .container(screenWidth / 2, screenHeight - 60)
      .setScrollFactor(0)
      .setDepth(30)
      .setVisible(false);
    this._checkpointBtn = this.add
      .image(-50, 0, "GJ_GameSheet03", "GJ_checkpointBtn_001.png")
      .setOrigin(0.5, 0.5)
      .setInteractive()
      .setScale(0.8);
    this._makeBouncyButton(this._checkpointBtn, 0.8, () => {
      if (
        ...
        this._practicedMode.saveCheckpoint(
          this._state,
          this._playerWorldX,
          this._cameraX,
          this,
    ...
    this._clearCheckpointBtn = this.add
      .image(50, 0, "GJ_GameSheet03", "GJ_removeCheckBtn_001.png")
      .setOrigin(0.5, 0.5)
      .setInteractive()
      .setScale(0.8);
    ...
    this._checkpointBtnContainer.add([
      this._checkpointBtn,
      this._clearCheckpointBtn,
    ]);
  ...
  _createPauseToggleButton(
    ...
    const pieceHeight = this.add
      .image(
        0,
        0,
        "GJ_GameSheet03",
        _0x268e2b ? "GJ_checkOn_001.png" : "GJ_checkOff_001.png",
      )
      .setScale(0.7)
      .setInteractive();
    const _0x15c0df = this.add
      .bitmapText(25 + 10, 0, "bigFont", _0xe203c3, 32)
      .setOrigin(0, 0.5);
    _0x4864cc.add([pieceHeight, _0x15c0df]);
    _0x5376fd.add(_0x4864cc);
    const _0x232e51 = (_0x1dce15) => {
      pieceHeight.setTexture(
        "GJ_GameSheet03",
        _0x1dce15 ? "GJ_checkOn_001.png" : "GJ_checkOff_001.png",
      );
  ...
  _buildPauseOverlay() {
    ...
    this._pauseContainer = this.add
      ...
      .setScrollFactor(0)
      .setDepth(100);
    const _0x505665 = this.add.rectangle(
      textureY,
      _0xf70e04,
      screenWidth,
      screenHeight,
    ...
    const _0x954813 = this._drawScale9(
      textureY,
      _0xf70e04,
      _0x4eb71b,
      650,
    ...
    const _0x426993 = this.add
      .image(
        textureY - _0x4eb71b / 2 + 40,
        60,
        "GJ_WebSheet",
        _0x3874ed
          ? "toggleFullscreenOff_001.png"
    ...
    this._pauseContainer.add(
      this.add
        .bitmapText(textureY, 65, "bigFont", window.currentlevel[1], 40)
        .setOrigin(0.5, 0.5),
    );
    const _0x21dacf = 170;
    const _0x46bab2 = this._bestPercent || 0;
    const _0x38b8d1 = this.add
      .image(textureY, _0x21dacf, "GJ_WebSheet", "GJ_progressBar_001.png")
      .setTint(0)
      .setAlpha(125 / 255);
    this._pauseContainer.add(_0x38b8d1);
    const _0x1d49a9 = this.textures.getFrame(
      "GJ_WebSheet",
    ...
    const _0x3d0987 = this.add
      .image(0, 0, "GJ_WebSheet", "GJ_progressBar_001.png")
      .setTint(65280)
      .setScale(0.992, 0.86)
      .setOrigin(0, 0.5)
      .setCrop(0, 0, _0x3782ca, _0x1e6502);
    _0x3d0987.setPosition(textureY - (_0xb5ab6f * 0.992) / 2, _0x21dacf);
    this._pauseContainer.add(_0x3d0987);
    this._pauseContainer.add(
      this.add
        .bitmapText(textureY, _0x21dacf, "bigFont", _0x46bab2 + "%", 30)
        .setOrigin(0.5, 0.5)
        .setScale(0.7),
    );
    this._pauseContainer.add(
      this.add
        .bitmapText(textureY, 130, "bigFont", "Normal Mode", 30)
        .setOrigin(0.5, 0.5)
        .setScale(0.78),
    );
    const _pausePractPct = this._practiceBestPercent || 0;
    const _pausePractBarY = 245;
    const _pausePractBarImg = this.add
      .image(textureY, _pausePractBarY, "GJ_WebSheet", "GJ_progressBar_001.png")
      .setTint(0)
      .setAlpha(125 / 255);
    this._pauseContainer.add(_pausePractBarImg);
    const _pausePractFrame = this.textures.getFrame(
      "GJ_WebSheet",
      "GJ_progressBar_001.png",
    );
    ...
    const _pausePractFillW = Math.max(
      1,
      Math.floor(_pausePractBarW * (_pausePractPct / 100)),
    );
    const _pausePractFg = this.add
      .image(0, 0, "GJ_WebSheet", "GJ_progressBar_001.png")
      .setTint(0x00ffff)
      .setScale(0.992, 0.86)
      .setOrigin(0, 0.5)
      .setCrop(0, 0, _pausePractFillW, _pausePractBarH);
    _pausePractFg.setPosition(
      textureY - (_pausePractBarW * 0.992) / 2,
      _pausePractBarY,
    );
    ...
    const _0x4791ac = [
      ...
      {
        frame: this._practicedMode.practiceMode
    ...
    const _0x25aa59 = _0x4791ac.map((_0x120c08) => {
      const _0x44c01c = this.textures.getFrame(
        _0x120c08.atlas,
        _0x120c08.frame,
      );
      if (_0x44c01c) {
    ...
    let _0x599a9b =
      textureY -
      (_0x25aa59.reduce((_0x53adf8, _0x10ae31) => _0x53adf8 + _0x10ae31, 0) +
        (_0x4791ac.length - 1) * 40) /
        2;
    for (let _0x18feee = 0; _0x18feee < _0x4791ac.length; _0x18feee++) {
      ...
      const _0x7f0786 = this.add
        .image(_0x599a9b + _0x228482 / 2, 390, _0x17809c.atlas, _0x17809c.frame)
        .setInteractive();
      if (_0x17809c.action === null) {
        ...
        this._makeBouncyButton(_0x7f0786, 1, () => {
          const isPracticeMode = this._practicedMode.togglePracticeMode();
          _0x7f0786.setTexture(
            "GJ_GameSheet03",
            isPracticeMode ? "GJ_normalBtn_001.png" : "GJ_practiceBtn_001.png",
          );
          _0x7f0786.setAngle(90).setFlipY(true);
          if (this._checkpointBtnContainer) {
            this._checkpointBtnContainer.setVisible(isPracticeMode);
    ...
    const _0xe34699 = (_0x422be3, _0x4b32e0, _0xaaab25, _0x169b87) => {
      this._pauseContainer.add(
        this.add
          .image(_0x422be3 - 180 - 5, _0x1008ae, "GJ_WebSheet", _0x4b32e0)
          .setScale(1.2),
      );
      ...
      const _0x43dbf4 = this.add
        .tileSprite(
          _0x34d1c1,
          _0x1008ae,
          _0xe86505 > 0 ? _0xe86505 : 1,
          11.2,
          "sliderBar",
        )
        .setOrigin(0, 0.5)
        .setVisible(_0xe86505 > 0);
      this._pauseContainer.add(_0x43dbf4);
      const _0x4de88c = this.add
        .image(_0x422be3, _0x1008ae, "GJ_WebSheet", "slidergroove.png")
        .setScale(_0x22b43a);
      this._pauseContainer.add(_0x4de88c);
      const _0x106f98 = _0x34d1c1 + _0xaaab25 * _0x51c57b;
      const sliderThumb = this.add
        .image(_0x106f98, _0x1008ae, "GJ_WebSheet", "sliderthumb.png")
        .setScale(_0x22b43a)
        .setInteractive({
          draggable: true,
          useHandCursor: true,
      ...
      sliderThumb.on("drag", (_0x1ac7f7, _0x35b64c) => {
        sliderThumb.x = Math.max(
          _0x34d1c1,
          Math.min(_0x34d1c1 + _0x51c57b, _0x35b64c),
        );
    ...
    _0xe34699(
      ...
      (_0x3ebce2) => this._audio.setUserMusicVolume(_0x3ebce2),
    );
    _0xe34699(
      textureY + 200,
      "GJ_sfxIcon_001.png",
    ...
    this._noclipCheckbox = this._createPauseToggleButton(
    ...
    this._showHitboxesCheckbox = this._createPauseToggleButton(
      this._pauseContainer,
      textureY - 100,
      600,
      "Hitbox",
    ...
    this._showPercentageCheckbox = this._createPauseToggleButton(
      this._pauseContainer,
      textureY + 100,
      600,
      "%",
    ...
    this._solidWaveCheckbox = this._createPauseToggleButton(
      this._pauseContainer,
      textureY + 250,
      600,
      "Solid",
  ...
  _buildInfoPopup() {
    ...
    const _0x249eb7 = this.add.rectangle(
      xPos,
      _0x4c3182,
      screenWidth,
      screenHeight,
    ...
    const _0x2c64c2 = this._drawScale9(
      xPos,
      _0x4c3182,
      480,
      _0xe2830b,
    ...
    const _0x5a0f88 = this.add
      .image(xPos - 240 + 20, 172, "GJ_WebSheet", "GJ_closeBtn_001.png")
      .setScale(0.8)
      .setInteractive();
    ...
    let yPos = 206;
    const _0x302fca = this.add
      .bitmapText(xPos, yPos, "bigFont", "Credits", 40)
      .setOrigin(0.5, 0.5);
    this._infoPopup.add(_0x302fca);
    yPos += 55;
    const _0x22e4c7 = this.add
      .bitmapText(xPos, yPos, "goldFont", "Made by RobTop Games", 40)
      .setOrigin(0.5, 0.5)
      .setScale(0.6);
    this._infoPopup.add(_0x22e4c7);
    yPos += 35;
    const _0x3cdf70a = this.add
      .bitmapText(xPos, yPos, "goldFont", "Modded by:", 40)
      .setOrigin(0.5, 0.5)
      .setScale(0.6);
    this._infoPopup.add(_0x3cdf70a);
    yPos += 35;
    const _0x3cdf70c = this.add
      .bitmapText(xPos, yPos, "goldFont", "AntiMatter, breadbb, bog, aloaf", 40)
      .setOrigin(0.5, 0.5)
      .setScale(0.6);
    this._infoPopup.add(_0x3cdf70c);
    yPos += 35;
    const _0x3cdf70b = this.add
      .bitmapText(xPos, yPos, "goldFont", "PinkDev, t0nchi7, arbstro", 40)
      .setOrigin(0.5, 0.5)
      .setScale(0.6);
    this._infoPopup.add(_0x3cdf70b);
    yPos += 35;
    const _0x3cdf70d = this.add
      .bitmapText(xPos, yPos, "goldFont", " and rohanis0000", 40)
      .setOrigin(0.5, 0.5)
      .setScale(0.6);
    this._infoPopup.add(_0x3cdf70d);
    yPos += 30;
    const _0x97b2a9 = this.add
      .text(xPos, 463, "© 2026 RobTop Games. All rights reserved.", {
        fontSize: "12px",
        color: "#000000",
        fontFamily: "Arial",
      })
  ...
  _buildUpdateLogPopup() {
    ...
    const cornerRadius =
      this.textures.get("GJ_square02").source[0].width * 0.325;
    const popupBg = this._drawScale9(
      0,
      0,
      480,
      popupWidth,
    ...
    const closeBtn = this.add
      .image(-240 + 20, -148, "GJ_WebSheet", "GJ_closeBtn_001.png")
      .setScale(0.8)
      .setInteractive();
    ...
    const title = this.add
      .bitmapText(0, -124, "bigFont", "BETA (EXPECT BUGS)", 30)
      .setOrigin(0.5, 0.5)
      .setTint(0xff6666);
    ...
    scrollFrameBg.fillRoundedRect(
      scrollAreaX - scrollAreaW / 2,
      scrollAreaY - scrollAreaH / 2,
      scrollAreaW,
      scrollAreaH,
      8,
    );
    bounceContainer.add(scrollFrameBg);
    const contentContainer = this.add.container(
      0,
      scrollAreaY - scrollAreaH / 2 + 8,
    );
    ...
    */
    ...
    updateEntries.forEach((entry) => {
      const txt = this.add
        .bitmapText(0, yPos, entry.font || "bigFont", entry.text, 32)
        .setOrigin(0.5, 0)
    ...
    const scrollZone = this.add
      .zone(scrollAreaX, scrollAreaY, scrollAreaW, scrollAreaH)
      .setInteractive();
  ...
  _expandHitArea(_0x122213, _0x37180a) {
    ...
    _0x122213.input.hitArea.setTo(
      -_0x960250,
      -_0x3f88a1,
      _0x46ea45 + _0x960250 * 2,
      _0x43b461 + _0x3f88a1 * 2,
    );
  }
  ...
  _drawScale9(
    ...
    for (let _0x24f653 = 0; _0x24f653 < _0x1d065e.length; _0x24f653++) {
      ...
      const _0x1145e5 = this.add
        .image(scale9Piece.dx, scale9Piece.dy, _0x24a44b, _0xade586)
        .setOrigin(0, 0)
        .setDisplaySize(scale9Piece.dw, scale9Piece.dh);
  ...
  _positionMenuItems() {
    ...
    if (this._downloadBtns) {
      ...
      for (
        let _0x1bdfae = 0;
        _0x1bdfae < this._downloadBtns.length;
        _0x1bdfae++
      ) {
        this._downloadBtns[_0x1bdfae].setPosition(
          _0x285ef7 - _0x1bdfae * _0x23d03e,
          _0x4a8263,
        );
      }
    }
  ...
  _respawnFromCheckpoint() {
    ...
    this._level._groundStartScreenY =
      checkpoint.groundStartScreenY !== undefined
        ? checkpoint.groundStartScreenY -
          (checkpoint.cameraY || 0) +
          this._cameraY
        : b(0) + this._cameraY;
    this._level._ceilingStartScreenY =
      checkpoint.ceilingStartScreenY -
      (checkpoint.cameraY || 0) +
      this._cameraY;
  ...
  _applyScreenResize() {
    ...
    if (!this._menuActive) {
      ...
      this._player.syncSprites(
        ...
        this._cameraY,
        0,
        this._getMirrorXOffset(_0xde8a1a),
      );
      this._applyMirrorEffect();
    }
  }
  ...
  update(_0x54fa47, deltaTime) {
    ...
    if (this._fpsAccum >= 250) {
      this._fpsText.setText(
        Math.round((this._fpsFrames * 1000) / this._fpsAccum),
      );
      this._fpsAccum = 0;
      this._fpsFrames = 0;
    }
    ...
    if (this._menuActive) {
      ...
      this._menuCameraX =
        (this._menuCameraX || 0) + menuDelta * c * d * menuSpeed;
    ...
    if (this._slideIn) {
      ...
      this._slideGroundX =
        ...
        slideDelta * c * d * slideGroundSpeed;
      ...
      this._player.syncSprites(
        ...
        this._cameraY,
        deltaTime / 1000,
        this._getMirrorXOffset(slidePlayerScreenX),
      );
      this._level.additiveContainer.x = -this._cameraX;
      this._level.additiveContainer.y = this._cameraY;
      this._level.container.x = -this._cameraX;
    ...
    this._level.updateEndPortalY(
      this._cameraY,
      this._state.isFlying || this._state.isWave || this._state.isUfo,
    );
    ...
    if (this._levelWon) {
      ...
      if (this._endCamTween) {
        ...
        this._cameraX =
          visMaxSection.fromX +
          (visMaxSection.toX - visMaxSection.fromX) * visMaxSection.p;
        this._cameraY =
          visMaxSection.fromY +
          (visMaxSection.toY - visMaxSection.fromY) * visMaxSection.p;
      }
      this._cameraXRef._v = this._cameraX;
      this._level.additiveContainer.x = -this._cameraX;
      this._level.additiveContainer.y = this._cameraY;
    ...
    if (this._state.isDead) {
      ...
      if (!this._newBestShown) {
        ...
        this._lastPercent = Math.min(
          99,
          Math.max(0, Math.floor((_0x169d53 / _0x435587) * 100)),
        );
        if (
        ...
        if (this._practicedMode.practiceMode) {
          const pracKey =
            "practiceBestPercent_" + (window.currentlevel[2] || "level_1");
          const prevPracticeBest = parseFloat(
            localStorage.getItem(pracKey) || "0",
          );
    ...
    this._orbParticleAngle =
      ((this._orbParticleAngle || 0) + deltaTime * 0.004) % (Math.PI * 2);
      ...
      if (this._level && this._level._orbSprites && this._level.container) {
        try {
          ...
          for (let _oSpr of this._level._orbSprites) {
            if (_drawn >= 4) break;
            if (!_oSpr || !_oSpr.visible || !_oSpr.active || !_oSpr.scene)
              continue;
            ...
            _drawn++;
            const _orbTint =
              _oSpr.tintTopLeft !== undefined && _oSpr.tintTopLeft !== 16777215
                ? _oSpr.tintTopLeft
                : window.mainColor;
            for (let _pi = 0; _pi < 5; _pi++) {
              ...
              const _ang =
                this._orbParticleAngle * _orbitSpeed + (_pi * Math.PI * 2) / 5;
    ...
    let subSteps =
      quantizedDelta > 0 ? Math.max(1, Math.round(quantizedDelta * 4)) : 0;
    if (subSteps > 60) {
      subSteps = 60;
    }
    ...
    for (let i = 0; i < subSteps; i++) {
      ...
      if (this._isDual && !this._state2.isDead) {
        ...
        this._player2.checkCollisions(
          this._playerWorldX - centerX - horizontalDelta,
        );
      ...
      if (this._level.endXPos > 0) {
        ...
        if (cameraOffsetX >= maxCameraX - 200) {
          ...
          const easeInOutCubic = (t) =>
            t < 0.5
              ? Math.pow(t * 2, easingPower) / 2
              : 1 - Math.pow((1 - t) * 2, easingPower) / 2;
          this._endCamTween = {
            p: 0,
            fromX: this._cameraX,
            toX: maxCameraX,
    ...
    this._applyMirrorEffect();
    this._percentageLabel.setText(
      `${((this._playerWorldX / (this._level.endXPos || 6000)) * 100).toFixed(2)}%`,
    );
  }
  _applyMirrorEffect() {
    const isMirrored = this._state.mirrored;
    const containers = [
      this._level.additiveContainer,
      this._level.container,
      this._level.topContainer,
    ];
    if (isMirrored) {
  ...
  _showNewBest() {
    ...
    let _0x12bde3 = this.add
      .image(0, 0, "GJ_WebSheet", "GJ_newBest_001.png")
      .setOrigin(0.5, 1);
    let _0x544c9c = this.add
      .bitmapText(0, 2, "bigFont", this._lastPercent + "%", 65)
      .setOrigin(0.5, 0)
      .setScale(1.1);
    let _0x326cb9 = this.add
      .container(_0x9f2437, 300, [_0x12bde3, _0x544c9c])
      .setScrollFactor(0)
      .setDepth(60)
      .setScale(0.01);
    this.tweens.add({
  ...
  _levelComplete() {
    ...
    for (let _0x481f7c = 0; _0x481f7c < 5; _0x481f7c++) {
      this.time.delayedCall(_0x481f7c * 50, () =>
        _s(
          this,
          _0x356782,
          _0x2d967b,
  ...
  _showCompleteEffect() {
    ...
    (function (_0x3f5321, _0x8f5267, _0x2f1e2d, _0x4b5e5b) {
      ...
      const _0x232789 =
        Math.round(Math.sqrt(screenWidth ** 2 + 102400)) + _0x29d856 * 32.5;
      const _0x1c105b = 180;
      const _0x586720 = 40;
      const _0x57b9ff = 195;
      const _0x2b6612 = 40;
      ...
      for (let _0x59890f = _0x44369e.length - 1; _0x59890f > 0; _0x59890f--) {
        ...
        [_0x44369e[_0x59890f], _0x44369e[_0x2bf73b]] = [
          _0x44369e[_0x2bf73b],
          _0x44369e[_0x59890f],
        ];
      }
      ...
      for (let _0x104cbb = 0; _0x104cbb < _0x1b2543; _0x104cbb++) {
        const _0x1a79fc =
          _0x104cbb * _0x57b9ff +
          _0x2b6612 +
          _0x5ce50e * (Math.random() * 2 - 1);
        const _0x6eb03a = _0x26b2b1 + _0x6f49c1 * (Math.random() * 2 - 1);
        const _0x2e9531 = _0x1c105b + _0x586720 * (Math.random() * 2 - 1);
        const _0x28e7b3 = Math.min(
          1,
          Math.max(0, _0x4da54f + _0x20decf * (Math.random() * 2 - 1)),
        );
        const _0x34147c =
          _0x44369e[_0x104cbb] + _0x323ded * Math.random() + 180;
        const containerY = _0x3f5321.add
          .graphics()
          .setScrollFactor(0)
          .setDepth(-1)
          .setBlendMode(S)
          .setPosition(_0x8f5267, _0x2f1e2d)
          .setAngle(_0x34147c)
          .setAlpha(_0x28e7b3)
          .setVisible(false);
        const _0x496d96 = {
      ...
      b(this._endPortalGameY) + this._cameraY,
      window.mainColor,
    );
  ...
  _showCompleteText() {
    ...
    const _0x45ab26 = this.add
      .image(_0x56628c, 250, "GJ_WebSheet", "GJ_levelComplete_001.png")
      .setScrollFactor(0)
      .setDepth(60)
      .setScale(0.01);
    this.tweens.add({
    ...
    for (let _0x5f16c8 = 0; _0x5f16c8 < 2; _0x5f16c8++) {
      this.add
        .particles(_0x56628c, 250, "GJ_WebSheet", {
          frame: "square.png",
          speed: {
            min: 300,
            max: 700,
    ...
    for (let _0x579e05 = 0; _0x579e05 < 5; _0x579e05++) {
      this.time.delayedCall(_0x579e05 * 50, () =>
        _s(
          this,
          _0x2eadf2,
          _0x380b24,
    ...
    for (let _0x429722 = 0; _0x429722 < 10; _0x429722++) {
      ...
      this.time.delayedCall(Math.max(0, _0xbf7dd0), () =>
        ws(this, window.mainColor, window.secondaryColor),
      );
    }
  ...
  _showEndLayer() {
    ...
    this._endLayerOverlay = this.add
      .rectangle(containerX, _0x1aa656, screenWidth, screenHeight, 0, 0)
      .setScrollFactor(0)
      .setDepth(200)
      .setInteractive();
    ...
    const _0x43f2e3 = this.textures.getFrame(
      "GJ_WebSheet",
    ...
    this._endLayerInternal.add(
      this.add
        .image(_0x2a115c - 40, 80, "GJ_WebSheet", "GJ_table_side_001.png")
        .setOrigin(0, 0)
        .setScale(1, _0x3feccc),
    );
    this._endLayerInternal.add(
      this.add
        .image(
          _0x2a115c + _0x595215 + 40,
          80,
          "GJ_WebSheet",
          "GJ_table_side_001.png",
        )
    ...
    const _0x33b564 = this.add.image(
      _0x2a115c + 356,
      70,
      "GJ_WebSheet",
    ...
    this._endLayerInternal.add(
      this.add
        .image(containerX - 312, _0x3e9c79, "GJ_WebSheet", "chain_01_001.png")
        .setOrigin(0.5, 1),
    );
    this._endLayerInternal.add(
      this.add
    ...
    this._endLayerInternal.add(
      this.add
        .image(containerX, 170, "GJ_WebSheet", "GJ_levelComplete_001.png")
        .setScale(0.8),
    );
    const _0x45b6e4 = 0.8;
    let _0xe44f6d = 250;
    const _0x2de55e = this.add
      .bitmapText(
        containerX,
        _0xe44f6d,
        "goldFont",
    ...
    _0x2be782 =
      _0x30687e > 0
        ? String(_0x30687e).padStart(2, "0") +
          ":" +
          String(_0x52f8ee).padStart(2, "0") +
          ":" +
          String(_0x2591d0).padStart(2, "0")
        : String(_0x52f8ee).padStart(2, "0") +
          ":" +
          String(_0x2591d0).padStart(2, "0");
    ...
    this._endLayerInternal.add(
      this.add
        .bitmapText(containerX, _0xe44f6d, "goldFont", "Time: " + _0x2be782, 40)
        .setOrigin(0.5, 0.5)
        .setScale(_0x45b6e4),
    );
    const _0x452429 = [
    ...
    this._endLayerInternal.add(
      this.add
        .bitmapText(containerX + _0x45540f, _0x241209, "bigFont", _0x165c06, 40)
        .setOrigin(0.5, 0.5)
        .setScale(0.8)
        .setCenterAlign(),
    );
    this._endLayerInternal.add(
      this.add
        .image(containerX - _0x45540f, 352.5, "GJ_WebSheet", "getIt_001.png")
        .setScale(1 / 1.5),
    );
    const _0x34b1bd = [
      {
    ...
    for (let _0x10f8cc = 0; _0x10f8cc < _0x34b1bd.length; _0x10f8cc++) {
      ...
      const _0x4c7fb8 = this.add
        .image(
          containerX + _0x1e3f82,
          437.5,
          "GJ_WebSheet",
          _0xd7310b.key + ".png",
        )
      ...
      this._makeBouncyButton(_0x4c7fb8, _0x55a82e, () =>
        window.open(_0xd7310b.url, "_blank"),
      );
    }
    _0x2de55e.width;
    ...
    for (const _0x2d4335 of _0x45fc2b) {
      const _0xdde774 = this.add
        .image(containerX + _0x2d4335.dx, 555, "GJ_WebSheet", _0x2d4335.frame)
        .setInteractive();
  ...
  _playStarAward() {
    ...
    const _0x453043 = this.add
      .image(_0x4edc03, _0x5a0e9, "GJ_WebSheet", "GJ_bigStar_001.png")
      .setScale(3)
      .setAlpha(0);
    ...
    this.time.delayedCall(100, () => {
      ...
      this.add
        .particles(_0x1204d3, _0x96e3b2, "GJ_WebSheet", {
          frame: "square.png",
          speed: {
            min: 200,
            max: 600,
  ...
  _hideEndLayer(_0x272eb1) {
    ...
    this.tweens.add({
      ...
      ease: (_0xc1c75) =>
        _0xc1c75 < 0.5
          ? Math.pow(_0xc1c75 * 2, 2) / 2
          : 1 - Math.pow((1 - _0xc1c75) * 2, 2) / 2,
      onUpdate: () => {
...
function ws(_0x13c75c, _0x23c5aa = 16777215, _0x52bb5b = 16777215) {
  ...
  _0x13c75c.add
    .particles(_0x4eb200, _0x126811, "GJ_WebSheet", {
      frame: "square.png",
      speed: {
        min: 520,
        max: 920,
...
function checkForAutoLoad() {
  ...
  const lastLoadTime = parseInt(
    localStorage.getItem("webdash_last_load_time") || "0",
  );
  const now = Date.now();
